...
EN = [
  ...
  ("mi(ic('image') + (IS_XHS ? '保存图片到相册' : '导出图片（png）')", "mi(ic('image') + (IS_XHS ? 'Save to album' : 'Export image (png)')"),
  ...
  ('>确定</button>', '>OK</button>'),
  ('<h3>粘贴备份 json 导入</h3>', '<h3>Paste backup json</h3>'),
...
JA += [
  ...
  ("mi(ic('image') + (IS_XHS ? '保存图片到相册' : '导出图片（png）')", "mi(ic('image') + (IS_XHS ? '保存图片到相册' : '画像を出力（png）')"),
